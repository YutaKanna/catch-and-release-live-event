        ...
        <div class="row">
            ...
            <div class="col-sm-8">
                <div class="row">
                    ...
                    <div class="col-sm-12">{{ $musicianGroup->name ?? '' }}</div>
                </div>
                <div class="row">
                ...
                <div class="row">
                {!! Form::open([
                    'route' => ['admin.musician_groups.store', $musicianGroup],
                    'method' => 'post',
                ]) !!}
                {{ csrf_field() }}
                    <div class="col-sm-12">ミュージシャンを招待する</div>
                    <div class="col-sm-12">
                        {!! Form::text('email', null, [
                                'class' => 'form-control'.($errors->has('email') ? ' is-invalid' : ''),
                        ]) !!}
                        <button type="submit" class="btn btn-primary" style="margin-top: 10px;">招待する</button>
                    </div>
                {!! Form::close() !!}
                </div>
            </div>
